...
if len(unique_scandinavian_artists) < len(scandinavian_artists):
    print(
        f"Removed {len(scandinavian_artists) - len(unique_scandinavian_artists)} duplicate artists.")

# Load the data from the existing top100_Artists.json
...
for artist_name in unique_scandinavian_artists:
    ...
    if artist_name not in top_artists:
        ...
        listeners = artist_info_data.get(
            "artist", {}).get("stats", {}).get("listeners")
        top_artists[artist_name] = {
            "listeners": int(listeners) if listeners else 0}


# Save the combined data to top100_w50scandi_artists
